...
\begin{align*}
	...
	\E[\varphi(Y)] &= \int_{\cY} g(y) P_Y(dx) = \int_{\cY} \varphi(y) p_Y(x) \nu_Y(dy)
\end{align*}
...
\begin{definition}
	...
	\end{equation*}
	namely a minimizer of the loss averaged by the posterior distribution, is called a \emph{Bayes} or \emph{Bayesian estimator} associated to the prior $\mu(\theta) = p(\theta) \lambda(d \theta)$ and to the loss $\ell$.
\end{definition}
