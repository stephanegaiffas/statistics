...
Let us go back to the statistical inference problems that we considered in Chapter~\ref{chap:statistical_inference}.
...
For the problems of estimation and testing, we can define a set $A$ of \emph{decisions}: for estimation, it is $A = \Theta \subset \R^d$ while for testing we have binary decisions, so that $A = \{ 0, 1 \}$.
