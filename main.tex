...
\marginnote[*-3]{Using the same letter for both the density of $\mu$ (namely $\theta \mapsto p(\theta)$) and the density of $P_\theta$ (namely $x \mapsto p(x | \theta$)) might sound like a bad idea, but it will lead to very nice notations in what follows, and it won't lead to any ambiguity.}
...
\sidenote{This is the main criticism with Bayesian methods: the computation of the posterior on complex models, the posterior distribution cannot be computed explicitely, and can require costely numericalapproximation, or algorithms that strongly depart from the original forulation of the model and prior. Citer LDA, Variational Bayes, etc..., voir Bishop}
...
\begin{equation*}
	\argmin_{t \in \R} \int_\Theta |t - \theta| p(\theta | x) \lambda(d \theta) = F_{x}^{-1}(1/2),
\end{equation*}
where $F_{x}^{-1}(1/2)$ is the \emph{median} of the posterior distribution, where we denoted as $F_x^{-1}$ the generalized inverse of the distribution function $F_x(\theta) = \int_{-\infty}^\theta p(\theta' | x) \lambda(d \theta')$.%
\sidenote{This comes from the fact that if $X$ is an integrable real random variable with distribution function $F$, a minimizer of $t \mapsto \E|X - t|$ is given by its median $F^{-1}(1/2)$.}

\begin{recipe}
	On some examples, we can compute explicitly the posterior distribution. 
	Given the data density $p(x | \theta)$ and the prior density $p(\theta)$, we simply write the joint distribution of $(X, T)$ and use the fact that the posterior density is a density, namely its integral with respect to $\theta$ must equal $1$:
	\begin{equation*}
		p(\theta | x) = \mathrm{constant}(x) p(x | \theta) p(\theta)
	\end{equation*}
	where $\mathrm{constant}(x) =  1 / \int_\Theta p(x | \theta) p(\theta) \lambda(d \theta)$. 
	Therefore, we can try to identify directly the posterior distribution by having a careful look at $p(x | \theta) p(\theta)$, together with some coffee, and looking for a density with respect to $\theta$.
\end{recipe}

Let us give some standard examples of prior and models (data distributions) for which the posterior and the Bayes estimator can be made explicit.

\paragraph{The Bayesian coin flip.}
