...
\begin{kaobox}[frametitle=Features engineering]
	...
	It is uncommon (and suspicious) to work directly with the raw \emph{features} vectors $X_1, \ldots, X_n$: a lot of effort is usually put on features engineering.
	...
	\marginnote{Large scale means that both $n$ and $d$ are large, when $n$ is large we are considering \emph{big data} while when $d$ is large we work with \emph{high-dimensional} features.}
	(such as web-display advertisement) are handled using simple linear models, but on highly tested and engineered features. We won't discuss this in this chapter, and will assume that $X_i$ are well-crafted features of vectors on which we want to train a linear model.	
...
While the previous contents is quite classical and well-known, the results provided in this Section is surprisingly recent and coming from the PhD manuscript of J. Mourtada, see~\sidecite{mourtadaphd,mourtada_leastsquares}.
...
\end{equation*}
If $\P_X$ is non degenerate and if $\Sigma = \E[ X X^\top] \succ 0$, it is easy to see that
\begin{equation*}
	\theta^\star = \argmin_{\theta \in \R^d} R(\theta) = \Sigma^{-1} \E[Y X].
\end{equation*}
...
\begin{align*}
	...
	&= \norm{\theta^\star - \theta}_\Sigma^2,
\end{align*}%
\marginnote[*-5]{We use here the fact that $Y = X^\top \theta^\star + \eps$ and that $\E[\eps | X] = 0$ almost surely.}%
where we introduce $\norm{x}_\Sigma^2 = x^\top \Sigma x$, which is a norm since $\Sigma \succ 0$ ($\P_X$ is non-degenerate).
...
\paragraph{Proof of Theorem~\ref{thm:minimax-leverage}} % (fold)
...
\section{Proof of Corollary~\ref{}} % (fold)
\label{sec:proof_of_corollary_}

% section proof_of_corollary_ (end)
\begin{proof}
