...
\todo{Parler aussi intervalle de prediction et intervalle de confiance}
...
\sidenote{We use the fact that $q_{\stu(k)}(\alpha) = - q_{\stu(k)}(1 - \alpha)$ in this construction, since $\stu(k)$ is a symmetrical distribution.}
...
If we are willing to assume that the model is Gaussian, namely $\beps \sim \nor(0, \sigma^2 \bI_n)$, and that the unknown label $Y_{\new}$ satisfies the same Gaussian linear model $Y_{\new} = X_{\new}^\top \theta + \eps_{\new}$ where $\eps_{\new}$ is independent of $\beps$ and
 ...
 can write
\begin{equation*}
	\wh Y_{\new} - Y_{\new} = X_{\new}^\top \wh \theta_n - (X_{\new}^\top \theta + \eps_{\new}) = X^\top (\bX^\top \bX)^{-1} \bX \beps - \eps
\end{equation*}

\todo{finish this and }
we can also build a prediction interval for $Y$.

% paragraph prediction_intervals (end)

\subsection{The Fisher test} % (fold)
\label{sub:the_fisher_test}

% subsection the_fisher_test (end)
...
\paragraph{Proof of Theorem~\ref{thm:least-squares-existence}.} % (fold)
...
\paragraph{Proof of Theorem~\ref{thm:cochran}} % (fold)
