...
\begin{equation}
	...
	\wh \theta_n \in \argmin_{\theta \in \R^d} \norm{Y - \bX \theta}^2 = \argmin_{\theta \in \R^d} \sum_{i=1}^n (Y_i - X_i^\top \theta)^2
\end{equation}
...
From now on, let us assume that $\E \norm{X_1}^2 < +\infty$ and $\E(Y_1^2) < +\infty$.
This allows to define 

C'est chiant... utiliser $X$ ou $\bX$ comment appeler le vecteur ?
