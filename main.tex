...
\sidenote{It is very easy to build a statistical test with $\alpha = 0$, namely zero Type~I error. For the cancer example from above, we just need to tell to all the patient that they have cancer. By doing so, we will never miss any cancer diagnostic. But arguably, this is not a good idea, so we need to give some slack in the construction of the test, by considering a small Type~I error $\alpha$, which allows to improve a lot the Type~II error.}
