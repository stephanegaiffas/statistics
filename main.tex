...
We are correct if $\vaprhi(X) = i$ whenever $\theta \in \Theta_i$ for $i \in \{ 0, 1 \}$and incorrect whenever  $\vaprhi(X) = 1 - i$ whenever $\theta \in \Theta_i$.
...
Ideally, we would like $\beta(\theta) \approx 0$ for $\theta \in \Theta_1$ and $\beta(\theta) \approx 1$ for $\theta \in ???$ \todo{correct} but this is impossible: if $\Theta$ is a connex set then $\Theta_0$ and $\Theta_1$ share a common frontier and $\beta$ must therefore be discontinuous on it.
